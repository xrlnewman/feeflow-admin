...
var (
	ErrNotFound         = errors.New("预算单据不存在")
	ErrAlreadyCompleted = errors.New("预算单据已经完成")
...
func NewService() *Service {
	...
	return &Service{
		...
		purchases: []PurchaseOrder{
			{ID: "PO20260716001", Supplier: "清和供应链", Warehouse: "东城中心仓", Items: 3, Amount: 12800, Status: "待审批", CreatedAt: now},
			{ID: "PO20260715008", Supplier: "织物工坊", Warehouse: "西郊备货仓", Items: 2, Amount: 7360, Status: "已入库", CreatedAt: now},
		},
		sales: []SalesOrder{
			{ID: "SO20260716032", Customer: "星河生活馆", Warehouse: "东城中心仓", Items: 8, Amount: 3280, Status: "待付款", CreatedAt: now},
...
func (s *Service) Dashboard(ctx context.Context) (Dashboard, error) {
	if err := ctx.Err(); err != nil {
		return Dashboard{}, fmt.Errorf("读取预算概览: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Warehouses(ctx context.Context) ([]Warehouse, error) {
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("读取成本中心: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Products(ctx context.Context, keyword string, page, pageSize int) ([]Product, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取费用单: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Alerts(ctx context.Context, page, pageSize int) ([]StockAlert, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取预算提醒: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Purchases(ctx context.Context, page, pageSize int) ([]PurchaseOrder, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取报销单: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Receive(ctx context.Context, id, idem string) (PurchaseOrder, error) {
	...
	for i := range s.purchases {
		...
		s.movements = append([]Movement{{ID: "MV" + time.Now().UTC().Format("20060102150405"), Product: "费用报销", Warehouse: s.purchases[i].Warehouse, Type: "入库", Quantity: s.purchases[i].Items, Source: id, CreatedAt: time.Now().UTC().Format(time.RFC3339)}}, s.movements...)
...
func (s *Service) Sales(ctx context.Context, page, pageSize int) ([]SalesOrder, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取付款单: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Ship(ctx context.Context, id, idem string) (SalesOrder, error) {
	...
	for i := range s.sales {
		...
		s.movements = append([]Movement{{ID: "MV" + time.Now().UTC().Format("20060102150406"), Product: "付款审批", Warehouse: s.sales[i].Warehouse, Type: "出库", Quantity: s.sales[i].Items, Source: id, CreatedAt: time.Now().UTC().Format(time.RFC3339)}}, s.movements...)
...
func (s *Service) Movements(ctx context.Context, page, pageSize int) ([]Movement, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取费用流水: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
