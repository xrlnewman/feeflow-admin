...
import (
	...
	"github.com/xrlnewman/feeflow-admin/server/internal/app/inventory"
	"github.com/xrlnewman/feeflow-admin/server/internal/platform/store"
)
...
func (s *Server) stockDashboard(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取预算概览失败", nil)
		return
	}
...
func (s *Server) stockWarehouses(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取成本中心失败", nil)
		return
	}
...
func (s *Server) stockProducts(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取费用单失败", nil)
		return
	}
...
func (s *Server) stockAlerts(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取预算提醒失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockPurchases(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取报销单失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockReceive(c *gin.Context) {
	...
	s.store.AddAudit(store.AuditLog{ID: uuid.NewString(), ActorID: claimsOf(c).UserID, Action: "expense_approve", Resource: c.Param("id"), Result: "success", CreatedAt: time.Now().UTC()})
}
...
func (s *Server) stockSales(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取付款单失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockShip(c *gin.Context) {
	...
	s.store.AddAudit(store.AuditLog{ID: uuid.NewString(), ActorID: claimsOf(c).UserID, Action: "payment_confirm", Resource: c.Param("id"), Result: "success", CreatedAt: time.Now().UTC()})
}
...
func (s *Server) stockMovements(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取费用流水失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockMutationError(c *gin.Context, err error, fallback string) {
	...
	case errors.Is(err, inventory.ErrNotFound):
		s.envelope(c, http.StatusNotFound, "预算单据不存在", nil)
	case errors.Is(err, inventory.ErrAlreadyCompleted):
		s.envelope(c, http.StatusConflict, "预算单据已经完成", nil)
