# FeeFlow Admin

FeeFlow 是企业费控后台与 Go Gin API，覆盖费用申请、发票归集、预算校验、审批、付款和费用分析。每次写操作都会记录状态事件与审计日志，重复提交使用 `Idempotency-Key` 安全重试。

## 关联仓库

- [FeeFlow Miniapp](https://github.com/xrlnewman/feeflow-miniapp)
- [个人博客项目页](https://field-notes-2fi.pages.dev/projects/feeflow-platform/)

## 技术栈与运行

Go 1.25 + Gin、MySQL 8.4、Redis 8、Vue 3 + Vite。执行 `docker compose -f deploy/docker-compose.yml up --build` 可启动完整 API、MySQL、Redis；未配置依赖时 API 自动回退内存演示数据。

```bash
go vet ./... && go test ./...
cd web && npm install && npm test && npm run build
```

## API 摘要

所有接口返回 `{ code, message, data, traceId }`，写接口支持 `Idempotency-Key`。费用状态依次为：费用申请 → 发票归集 → 预算校验 → 多级审批 → 付款登记 → 月度分析。

| Method | URL | 用途 |
| --- | --- | --- |
| GET | `/api/v1/dashboard` | 报销、审批、付款与预算指标 |
| GET | `/api/v1/warehouses` | 成本中心与预算池 |
| GET | `/api/v1/products` | 费用单分页与关键词搜索 |
| GET | `/api/v1/stocks/alerts` | 预算超支提醒 |
| POST | `/api/v1/purchase-orders/:id/receive` | 审批完成 |
| POST | `/api/v1/sales-orders/:id/ship` | 付款确认 |

默认演示管理员：`13900000000` / `demo123456`，仅用于本地预览。

MIT © xrlnewman
